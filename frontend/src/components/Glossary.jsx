...
const GLOSSARY_TERMS = [
  ...
  { term: 'RDP (Remote Desktop Protocol)', def: 'A protocol that lets you view and control another computer\'s screen over a network. Commonly used by IT staff to remotely manage Windows machines. Uses port 3389.' },
  ...
  { term: 'SMTP (Simple Mail Transfer Protocol)', def: 'The protocol used to send email between servers and from email clients to a mail server. Uses port 25 (server-to-server) or port 587 (client-to-server). Receiving email uses different protocols — IMAP or POP3.' },
  { term: 'SSH (Secure Shell)', def: 'A protocol for securely logging into and controlling remote computers over a network. All SSH traffic is encrypted, so capturing packets won\'t reveal the contents. Uses port 22.' },
